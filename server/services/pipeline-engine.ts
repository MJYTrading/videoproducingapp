...
const STEP_CONFIG: Record<number, {
  ...
  2:  { dependsOn: [1],        timeout: 600_000,    maxRetries: 3, retryDelays: [5_000, 15_000, 30_000] },
  3:  { dependsOn: [1],        timeout: 120_000,    maxRetries: 3, retryDelays: [5_000, 15_000, 30_000] },
  4:  { dependsOn: [2, 5],     timeout: 600_000,    maxRetries: 3, retryDelays: [5_000, 15_000, 30_000] },
  5:  { dependsOn: [2, 3],        timeout: 180_000,    maxRetries: 3, retryDelays: [5_000, 15_000, 30_000] },
  ...
  13: { dependsOn: [10],            timeout: 1_800_000,  maxRetries: 2, retryDelays: [10_000, 30_000] },  // 30 min timeout
  ...
  16: { dependsOn: [10, 12, 13, 15],       timeout: 300_000,    maxRetries: 2, retryDelays: [10_000, 30_000] },
...
function getExecutorName(stepNumber: number): string {
  const map: Record<number, string> = {
    ...
    12: 'Sonar + TwelveLabs', 13: 'Video Download API', 14: 'Elevate', 15: 'Elevate',
...
export async function startPipeline(projectId: string, fromQueue: boolean = false): Promise<{ success: boolean; error?: string }> {
  ...
  // ── Auto-skip disabled stappen op basis van enabledSteps ──
  const enabledSteps: number[] = project.enabledSteps ? JSON.parse(project.enabledSteps) : [];
  const hasEnabledSteps = Array.isArray(enabledSteps) && enabledSteps.length > 0;
  ...
  for (const stepNum of STEP_ORDER) {
    ...
    if (hasEnabledSteps && !enabledSteps.includes(stepNum)) {
      ...
      await updateStepInDb(projectId, stepNum, { status: 'skipped' });
      await addLog(projectId, 'info', stepNum, 'App',
        `${getStepName(stepNum)} overgeslagen (uitgeschakeld in project config)`);
      continue;
    }
    
    // Skip stappen die 'skip' als executor hebben in STEP_EXECUTOR_MAP (niet ready)
    const executorMapping = STEP_EXECUTOR_MAP[stepNum];
    if (executorMapping === 'skip') {
      state.skippedSteps.add(stepNum);
      await updateStepInDb(projectId, stepNum, { status: 'skipped' });
      await addLog(projectId, 'info', stepNum, 'App',
        `${getStepName(stepNum)} overgeslagen (nog niet geïmplementeerd)`);
      continue;
    }
  }

  // ── Stap 0 (Ideation) en 1 (Formulier) direct completed ──
  // Deze stappen zijn al klaar als de pipeline start (het formulier is ingevuld)
  for (const autoStep of [0, 1]) {
    if (!state.completedSteps.has(autoStep) && !state.skippedSteps.has(autoStep)) {
      state.completedSteps.add(autoStep);
      await updateStepInDb(projectId, autoStep, { status: 'completed', duration: 0 });
      await addLog(projectId, 'info', autoStep, 'App',
        `${getStepName(autoStep)} automatisch voltooid`);
    }
  }
