/**
 * PipelineListView v4.3
 * 
 * - Variabelen panel NAAST textarea (niet eronder)
 * - Klik = insert op cursor positie
 * - Project + Channel + Upstream + Datum variabelen
 * - Grotere, gebruiksvriendelijke velden
 * - Drag & drop reordering
 * - Skeleton toggle + test knop
 */
    ...
    <div className="max-w-5xl mx-auto py-6 px-4">
      ...
      <div className="space-y-1.5">
        {sortedNodes.map((node, idx) => (
          <div key={node.id} draggable onDragStart={() => handleDragStart(node.id)} onDragOver={(e) => handleDragOver(e, node.id)} onDragLeave={handleDragLeave} onDrop={() => handleDrop(node.id)} onDragEnd={handleDragEnd}
            className={dragOverId === node.id && dragId !== node.id ? 'border-t-2 border-brand-400' : ''}>
            <StepCard node={node} index={idx} total={sortedNodes.length}
              isOpen={openNodeId === node.id} isDragging={dragId === node.id}
...
interface StepCardProps {
  node: PipelineNodeData; index: number; total: number; isOpen: boolean; isDragging: boolean;
...
function StepCard({ node, index, total, isOpen, isDragging, onToggle, onMove, onRemove, upstreamData, models, voices, styles, colorGrades, pipelineId, onSave, saving, setSaving }: StepCardProps) {
  ...
  // Insert variable at cursor position in active prompt
  ...
  return (
    <div className={'rounded-xl border transition-all ' + (isDragging ? 'opacity-40 border-brand-400/50 ' : '') +
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 cursor-pointer select-none" onClick={onToggle}>
        <div className="cursor-grab active:cursor-grabbing text-zinc-600 hover:text-zinc-400 shrink-0" onClick={e => e.stopPropagation()}><GripVertical className="w-4 h-4" /></div>
      ...
      {isOpen && (
        <div className="px-4 pb-4 pt-1 border-t border-white/[0.04] space-y-4">
          ...
          {/* Test result */}
          {testResult && (
            <div className="bg-surface-200 rounded-lg p-3 max-h-[200px] overflow-auto">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] text-zinc-500 font-semibold uppercase">Test Resultaat</span>
                <button onClick={() => setTestResult(null)} className="text-zinc-600 hover:text-zinc-300"><X className="w-3 h-3" /></button>
              </div>
              <pre className="text-[10px] text-zinc-400 font-mono whitespace-pre-wrap">{testResult}</pre>
            </div>
          )}
          ...
          {isLLM && (
            <div className="space-y-4">
              ...
              {/* Prompt + Variables split layout */}
              <div className="flex gap-4">
                {/* Left: Prompts (takes most space) */}
                <div className="flex-1 min-w-0 space-y-3">
                  {/* Prompt tabs */}
                  <div className="flex gap-1">
                    <button onClick={() => setActivePrompt('system')}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${activePrompt === 'system' ? 'bg-brand-600/20 text-brand-300 ring-1 ring-brand-500/30' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/[0.04]'}`}>
                      System Prompt
                    </button>
                    <button onClick={() => setActivePrompt('user')}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${activePrompt === 'user' ? 'bg-brand-600/20 text-brand-300 ring-1 ring-brand-500/30' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/[0.04]'}`}>
                      User Prompt
                    </button>
                  </div>
                  ...
                  {activePrompt === 'system' ? (
                    <div>
                      <textarea ref={systemRef}
                        className="input-base font-mono text-[12px] min-h-[350px] w-full !py-4 !px-4 leading-relaxed resize-y"
                        placeholder="System prompt — definieert de rol en regels voor de LLM...&#10;&#10;Gebruik variabelen uit het panel rechts om dynamische data in te voegen."
                ...
                <div className="w-[260px] shrink-0 bg-surface-200/50 rounded-xl border border-white/[0.04] overflow-hidden">
                  ...
                  <div className="p-3 max-h-[360px] overflow-auto space-y-3">
                    ...
                    {upstreamData.length > 0 && (
                      <VarGroup icon={<Database className="w-3 h-3" />} title="Pipeline Data" color="text-brand-400" defaultOpen>
                        {(() => {
                    ...
                    {/* Project data */}
                    <VarGroup icon={<FolderOpen className="w-3 h-3" />} title="Project Data" color="text-emerald-400" defaultOpen>
                      {PROJECT_VARS.map(v => (
                    ...
                    {/* Channel data */}
                    <VarGroup icon={<Hash className="w-3 h-3" />} title="Kanaal Data" color="text-blue-400" defaultOpen={false}>
                      {CHANNEL_VARS.map(v => (
                    ...
                    {/* Date/time */}
                    <VarGroup icon={<Calendar className="w-3 h-3" />} title="Datum & Tijd" color="text-amber-400" defaultOpen={false}>
                      {DATE_VARS.map(v => (
...
function VarGroup({ icon, title, color, defaultOpen = true, children }: { icon: React.ReactNode; title: string; color: string; defaultOpen?: boolean; children: React.ReactNode }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div>
      <button onClick={() => setOpen(!open)} className="flex items-center gap-1.5 mb-1.5 w-full text-left group">
        <span className={color}>{icon}</span>
        <span className={'text-[9px] font-bold uppercase tracking-wider ' + color}>{title}</span>
        {open ? <ChevronDown className="w-3 h-3 text-zinc-600 ml-auto group-hover:text-zinc-400" /> : <ChevronRight className="w-3 h-3 text-zinc-600 ml-auto group-hover:text-zinc-400" />}
      </button>
      {open && <div className="space-y-0.5 ml-0.5">{children}</div>}
    </div>
  );
}
    ...
    case 'text':
      return (
        <div>
          ...
          <input className="input-base text-sm !py-2.5" value={value || ''} onChange={e => onChange(e.target.value)} placeholder={field.description} />
        </div>
      );
    case 'number':
      return (
        <div>
          <label className="text-xs text-zinc-400 mb-1.5 block font-medium">{field.label}</label>
          <input className="input-base text-sm !py-2.5" type="number" value={value ?? field.default ?? ''} onChange={e => onChange(parseFloat(e.target.value))} min={field.min} max={field.max} step={field.step} />
        </div>
      );
    case 'range':
      return (
        <div>
          <div className="flex justify-between mb-1.5">
            ...
            <span className="text-xs text-brand-300 font-mono">{value ?? field.default}</span>
          </div>
          <input type="range" className="w-full accent-brand-500 h-2" value={value ?? field.default ?? 0} onChange={e => onChange(parseFloat(e.target.value))} min={field.min ?? 0} max={field.max ?? 1} step={field.step ?? 0.1} />
        </div>
      );
    case 'select':
      return (
        <div>
          <label className="text-xs text-zinc-400 mb-1.5 block font-medium">{field.label}</label>
          <select className="input-base text-sm !py-2.5" value={value ?? field.default ?? ''} onChange={e => onChange(e.target.value)}>
      ...
      return (
        <div className="flex items-center justify-between col-span-2 py-2">
          <div>
            ...
            {field.description && <p className="text-[10px] text-zinc-600 mt-0.5">{field.description}</p>}
          </div>
          <button onClick={() => onChange(!(value ?? field.default))} className={'w-11 h-6 rounded-full relative transition ' + ((value ?? field.default) ? 'bg-brand-500' : 'bg-zinc-700')}>
            <div className={'w-5 h-5 rounded-full bg-white absolute top-0.5 transition-transform ' + ((value ?? field.default) ? 'translate-x-5' : 'translate-x-0.5')} />
          </button>
        </div>
      );
    case 'textarea':
      return (
        <div className="col-span-2">
          <label className="text-xs text-zinc-400 mb-1.5 block font-medium">{field.label}</label>
          <textarea className="input-base font-mono text-xs min-h-[120px] !py-3 !px-4 resize-y" value={value || ''} onChange={e => onChange(e.target.value)} placeholder={field.description} />
        </div>
      );
    case 'json':
      return (
        <div className="col-span-2">
          <label className="text-xs text-zinc-400 mb-1.5 block font-medium">{field.label}</label>
          <textarea className="input-base font-mono text-xs min-h-[100px] !py-3 !px-4 resize-y" value={typeof value === 'string' ? value : JSON.stringify(value || field.default, null, 2)} onChange={e => onChange(e.target.value)} />
        </div>
      );
    default: return null;
  }
...
function Chk({ label, checked, set }: { label: string; checked: boolean; set: (v: boolean) => void }) {
  return (
    <label className="flex items-center gap-2.5 cursor-pointer group">
      <input type="checkbox" checked={checked} onChange={e => set(e.target.checked)} className="rounded w-4 h-4 border-zinc-600 bg-surface-200 text-brand-500 focus:ring-brand-500/30" />
...
function MiniInp({ label, value, set, w }: { label: string; value: string; set: (v: string) => void; w: string }) {
  return (
    <div>
      ...
      <input className={'input-base text-sm !py-2 ' + w} value={value} onChange={e => set(e.target.value)} />
    </div>
  );
}
